...
Gauss Seidel em C++. Feito por Victor Martins e Daniel
...
Vetor* seidel(double* matriz[], double* b, double tol, const int n, int limite = 50){
    Vetor* x = new Vetor(n);  // Inicializo o vetor x
    double v[n], d; // Inicializo o vetor v (vetor x anterior) e a distância d (teste de parada)
    double somatorio = 0;
    for(int i = 0; i < n; i++) { x->resultado[i] = 0; v[i] = 0; }
    ...
    for(int k = 1; k < limite+1; k++){
        for (int i = 0; i < n; i++){
            ...
            // Vou preenchendo novo vetor x já recebendo valores anteriores em
            // tempo real para a próxima iteração
            x->resultado[i] = b[i] - somatorio;
        }
        
        // Fechou o vetor x, agora faz o teste de parada e compara com o vetor
        // anterior.
        d = teste_parada(x->resultado, v, n);

        if(d <= tol){ return x; } // Se for menor que a tolerância, acabou.
        // Se não, continua o método.

        // Faço uma cópia do vetor x pro vetor anterior. Pois o novo x será
        // modificado e o atual virará o anterior.
        for(int copia = 0; copia < n; copia++){
            v[copia] = x->resultado[copia];
        }
    }
...
int main(){
    cout << "PROGRAMA QUE CALCULA O VETOR DE SOLUÇÕES 'd' DE UM SISTEMA LINEAR ATRAVÉS DO MÉTODO NORMAL OU PELA INVERSA DA MATRIZ\n\n" << endl;
    ...
    if (input == 1){
        ...
        for(int i = 0; i < n; i++){
            ...
            Vetor* x = seidel(deepcopy_matriz(matriz, n).matriz, vetor_identidade, tol, n);
            ...
            for (int j = 0; j < n; j++){
                matriz_inversa[j][i] = x->resultado[j];
            }
        }
     ...
     /* CÁLCULO DO VETOR d PELA MATRIZ ORIGINAL */
        ...
        Vetor* x = seidel(deepcopy_matriz(matriz, n).matriz, b, tol, n);
        ...
        for(int i = 0; i < n; i++){
            if(i == n-1){ // Se for o último elemento
                cout << x->resultado[i]; // Não printa a vírgula
            }
            else{
                cout << x->resultado[i] << ", ";
            }
        ...
        // Deletando o vetor de resultados e o objeto x
        delete x;
